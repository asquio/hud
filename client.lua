...
AddEventHandler("ui:update", function(nbPlayerTotal)
	SendNUIMessage({
		type = "online-count",
		onlineCount = "ID : "..GetPlayerServerId(PlayerId())
	})
end)
...
Citizen.CreateThread(function()
	...
	cash = ESX.Math.GroupDigits(ESX.PlayerData.money)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	cash = ESX.Math.GroupDigits(xPlayer.money)
...
AddEventHandler('es:activateMoney', function(money)
    SendNUIMessage({
		...
		infos = {
			{
				name = 'money',
				value = ESX.Math.GroupDigits(money)
			}
		}
	})
end)
